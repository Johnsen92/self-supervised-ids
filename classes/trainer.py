...
class SupvervisedTrainer():
    ...
    def train(self):
        ...
        if self.cache.disabled or not os.path.isfile(chache_file_name):
            ...
            for epoch in range(self.epochs):
                ...
                for data, labels, categories in self.training_data: 
                    ...
                    if i % monitoring_interval == 0:
                        ...
                        time_left = sample_time * float(n_batches * self.epochs - epoch * n_batches - i)/3600.0
                        time_left_h = math.floor(time_left)
                        time_left_m = math.floor((time_left - time_left_h)*60.0)
                        print (f'Epoch [{epoch+1}/{self.epochs}], Step [{i+1}/{n_batches}], Avg. Loss: {avg_loss:.4f}, Time left: {time_left_h} h {time_left_m} m')
            ...
            self.cache.save('stats', self.stats)
            print('done')
        else:
            # Load cached model
            ...
            self.stats = self.cache.load('stats')
            print('done')
